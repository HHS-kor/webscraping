...
def create_soup(url):  # url받아 soup 객체 리턴
    headers = {
...
def print_news(index, title, link):  # 뉴스 출력
    print("{}. {}".format(index+1, title))
    print("  (링크 : {})".format(link))


def scrape_weather():  # 날씨
    print("[오늘의 인천 날씨]")
...
def scrape_headline_news():  # 뉴스
    print("[헤드라인 뉴스]")
    url = "https://news.naver.com"
    soup = create_soup(url)
    news_list = soup.find(
        "ul", attrs={"class": "hdline_article_list"}).find_all("li", limit=3)  # 3개까지만
...
def scrape_it_news():  # IT뉴스
    print("[IT 뉴스]")
    url = "https://news.naver.com/main/list.nhn?mode=LS2D&mid=shm&sid1=105&sid2=230"
    soup = create_soup(url)
    news_list = soup.find(
        "ul", attrs={"class": "type06_headline"}).find_all("li", limit=3)  # 3개까지만
    for index, news in enumerate(news_list):
        a_idx = 0
        img = news.find("img")
        if img:
            a_idx = 1  # img태그가 있으면 1번째 a태그 정보 활용
...
def scrape_english():  # 오늘의 영어회화
    ...
    print("(ENG)")
    # 8문장이 있다고 가정할 때, 4~7이 영어 (중간부분부터 끝까지)
    ...
    print("(KOR)")
    # 8문장이 있다고 가정할 때, 0~3이 영어 (처음부터 중간부분 직전까지)
    for sentence in sentences[:len(sentences)//2]:
        print(sentence.get_text().strip())
...
if __name__ == "__main__":  # 프로그램의 시작점일때만
    print(datetime.datetime.now().strftime('\t<%Y-%m-%d %H:%M:%S>'))
    print()
    scrape_weather()  # 날씨정보 가져오기
    print()
    scrape_headline_news()  # 뉴스 가져오기
    print()
