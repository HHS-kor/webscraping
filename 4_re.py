import re


def print_match(m):
    if m:
        print("m.group()", m.group())  # 일치하는 문자열 반환
        print("m.string", m.string)  # 입력받은 문자열 그대로 반환
        print("m.start()", m.start())  # 일치하는 문자열의 시작 index 반환
        print("m.end()", m.end())  # 일치하는 문자열의 끝 index
        print("m.span()", m.span())  # 일치하는 문자열의 시작 / 끝 index 반환

    else:
        print("메칭되지 않음")


p = re.compile("ca.e")  # . : 하나의 문자  /  ^ : 문자열 시작 (^de) /  $ : 문자열 끝 (se$)

# match : 주어진 문자열의 처음부터 일치하는지 확인, 뒷부분에 다른것 있어도 됨 ex)careless
m = p.match("careless")
print_match(m)

# search : 주어진 문자열 중에 일치하는게 있는지 확인
n = p.search("good care")
print_match(n)

lst = p.findall("good care cafe")  # findall : 일치하는 모든 것을 리스트 형태로 반환
print(lst)


# 1. p = re.compile("원하는 형태")
# 2. m = p.match("비교할 문자열") -> 처음부터 일치하는지 확인
# 3. m = p.search("비교할 문자열") -> 문자열 중 일치하는 것이 있는지 확인
# 4. lst = p.findall("비교할 문자열") -> 일치하는 모든 것들을 리스트 형태로 변환
#
# 원하는 형태 : 정규식
# . (ca.e) : 하나의 문자 care, cafe case (O) | caffe(X)
# ^ (^de) : 문자열의 시작 desk, destination (O) | fade(X)
# $ (se$) : 문자열의 끝 case, base (O) | face (x)
